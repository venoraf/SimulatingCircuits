		...
		for (int i = 0; i < steps; i++)
		{
			...
			else if (strcmp(ops[0], "DECODER") == 0)
			{
				...
				for (int x = 0; x < inps; x++) {
					opval += power(2, x) * getData(ops[1 + inps - x], inx);
			...
			else if (strcmp(ops[0], "MULTIPLEXER") == 0)
			{
				...
				int opval = 0;
				for (int x = 0; x < mux; x++) {
					int bit = getData(top[mux - x - 1], inx);
					opval += power(2, x) * bit;
					if (logLevel == 0) printf("bit %s -> %d as %d\n", top[mux - x - 1], bit, opval);
				}
				for (int x = 0; x < mux; x++)
				{
