...
int main(int argc, char* argv[])
{
	...
	if (solveGates(0) == 0)
	{
		...
		{
			int j = 0;
			int jCt = parseStep(circuit[i], tmp);
			//Replace Gates
			while (j < jCt) {
				if (tmp[0][0] == 'G') {
					int gateNo = getInxFromName(tmp[0]) - 1;
					strcpy(tmp[0], unknown[gateNo]);
					if (strcmp(tmp[0], "MULTIPLEXER") == 0) 
						strcpy(tmp[1], "4");
					else if (strcmp(tmp[0], "DECODER") == 0)
						strcpy(tmp[1], "2");
					else
						strcpy(tmp[1], "");
				}
			...
			while (j < jCt) {
				if (j == 0)
					strcpy(circuit[i], tmp[0]);
				else
					strcat(circuit[i], tmp[j]);
				if (strlen(tmp[j]) > 0) strcat(circuit[i], " ");
				j++;
			}
		}
		...
		do {
			flag = 0;
			//Reduce Circuit Associative
			int i = 0;
			while (i < steps - 1) {
			        parseStep(circuit[i], tmp);
				int srow = -1;
				if (strcmp(tmp[0], "OR") == 0) {
					for (int j = i + 1; j < steps; j++) {
						int jCt1 = parseStep(circuit[j], tmp1);
						if (strcmp(tmp1[0], "OR") == 0 &&
							((strcmp(tmp[1], tmp1[1]) == 0 && strcmp(tmp[3], tmp1[2]) == 0) ||
								(strcmp(tmp[1], tmp1[2]) == 0 && strcmp(tmp[3], tmp1[1]) == 0) ||
								(strcmp(tmp[2], tmp1[1]) == 0 && strcmp(tmp[3], tmp1[2]) == 0) ||
								(strcmp(tmp[2], tmp1[2]) == 0 && strcmp(tmp[3], tmp1[1]) == 0))) {
							srow = j;
						}
						else {
							for (int x = 1; x < jCt1 - 2; x++) {
								if (strcmp(tmp[3], tmp1[x]) == 0) {
									//out is used cannot change
									srow = -1;
								}
							}
						}
					...
					if (srow >= 0) {
						if (logLevel == 0) printf("found OR at %d ASSOC at %d\n", i, srow);
					        parseStep(circuit[srow], tmp1);
						if ((strcmp(tmp[1], tmp1[1]) == 0 && strcmp(tmp[3], tmp1[2]) == 0) || 
							(strcmp(tmp[1], tmp1[2]) == 0 && strcmp(tmp[3], tmp1[1]) == 0) || 
							(strcmp(tmp[2], tmp1[1]) == 0 && strcmp(tmp[3], tmp1[2]) == 0) ||
							(strcmp(tmp[2], tmp1[2]) == 0 && strcmp(tmp[3], tmp1[1]) == 0)) {
							strcpy(circuit[srow], "OR ");
							strcat(circuit[srow], tmp[1]);
							strcat(circuit[srow], " ");
							strcat(circuit[srow], tmp[2]);
							strcat(circuit[srow], " ");
							strcat(circuit[srow], tmp1[3]);
							for (int x = i; x < steps - 1; x++) {
								strcpy(circuit[x], circuit[x + 1]);
							}
							steps--;
							strcat(circuit[steps], "");
							flag = 1;
							break;
						}
						flag = 1;
			...
			i = 0;
			while (flag == 0 && i < steps - 2)
			{
				// Distributive needs 3 steps
				if (i < steps - 2) {
				        parseStep(circuit[i], tmp);
				        parseStep(circuit[i + 1], tmp1);
				        parseStep(circuit[i + 2], tmp2);

					if (((strcmp(tmp[0], "AND") == 0 && strcmp(tmp2[0], "OR")) || (strcmp(tmp[0], "OR") == 0 || strcmp(tmp2[0], "AND"))) &&
						strcmp(tmp[0], tmp1[0]) == 0 && strcmp(tmp[0], tmp2[1]) != 0 && // first to ops match and different from third
						strcmp(tmp[1], tmp1[1]) == 0 && strcmp(tmp[2], tmp1[2]) != 0 && // first args match and second args are diff 
						((strcmp(tmp[3], tmp2[1]) == 0 && strcmp(tmp1[3], tmp2[2]) == 0) ||
							(strcmp(tmp[3], tmp2[2]) == 0 && strcmp(tmp1[3], tmp2[1]) == 0))) {
						strcpy(circuit[i], tmp2[0]);
						strcat(circuit[i], " ");
						strcat(circuit[i], tmp[2]);
						strcat(circuit[i], " ");
						strcat(circuit[i], tmp1[2]);
						strcat(circuit[i], " ");
						strcat(circuit[i], tmp1[3]);

						strcpy(circuit[i + 1], tmp[0]);
						strcat(circuit[i + 1], " ");
						strcat(circuit[i + 1], tmp[1]);
						strcat(circuit[i + 1], " ");
						strcat(circuit[i + 1], tmp1[3]);
						strcat(circuit[i + 1], " ");
						strcat(circuit[i + 1], tmp2[3]);

						strcpy(circuit[i + 2], "");
						for (int x = i + 2; x < steps - 1; x++)
							strcpy(circuit[x], circuit[x + 1]);
						steps--;
						strcpy(circuit[steps], "");
						i = 0;
						flag = 1;
					}
					else
						if (((strcmp(tmp[0], "AND") == 0 && strcmp(tmp2[0], "OR")) || (strcmp(tmp[0], "OR") == 0 || strcmp(tmp2[0], "AND"))) &&
							strcmp(tmp[0], tmp1[0]) == 0 && strcmp(tmp[0], tmp2[0]) != 0 && // first to ops match and different from third
							strcmp(tmp[1], tmp1[2]) == 0 && strcmp(tmp[2], tmp1[1]) != 0 && // first args match and second args are diff 
							((strcmp(tmp[3], tmp2[1]) == 0 && strcmp(tmp1[3], tmp2[2]) == 0) ||
								(strcmp(tmp[3], tmp2[2]) == 0 && strcmp(tmp1[3], tmp2[1]) == 0))) {
							strcpy(circuit[i], tmp2[0]);
							strcat(circuit[i], " ");
							...
							if (((strcmp(tmp[0], "AND") == 0 && strcmp(tmp2[0], "OR")) || (strcmp(tmp[0], "OR") == 0 || strcmp(tmp2[0], "AND"))) &&
								strcmp(tmp[0], tmp1[0]) == 0 && strcmp(tmp[0], tmp2[0]) != 0 && // first to ops match and different from third
								strcmp(tmp[2], tmp1[1]) == 0 && strcmp(tmp[1], tmp1[2]) != 0 && // first args match and second args are diff 
								((strcmp(tmp[3], tmp2[1]) == 0 && strcmp(tmp1[3], tmp2[2]) == 0) ||
									(strcmp(tmp[3], tmp2[2]) == 0 && strcmp(tmp1[3], tmp2[1]) == 0))) {
								strcpy(circuit[i], tmp2[0]);
								strcat(circuit[i], " ");
								strcat(circuit[i], tmp[1]);
								strcat(circuit[i], " ");
								strcat(circuit[i], tmp1[2]);
								strcat(circuit[i], " ");
								strcat(circuit[i], tmp1[3]);
								...
								if (((strcmp(tmp[0], "AND") == 0 && strcmp(tmp2[0], "OR")) || (strcmp(tmp[0], "OR") == 0 || strcmp(tmp2[0], "AND"))) &&
									strcmp(tmp[0], tmp1[0]) == 0 && strcmp(tmp[0], tmp2[0]) != 0 && // first to ops match and different from third
									strcmp(tmp[2], tmp1[2]) == 0 && strcmp(tmp[1], tmp1[1]) != 0 && // first args match and second args are diff 
									((strcmp(tmp[3], tmp2[1]) == 0 && strcmp(tmp1[3], tmp2[2]) == 0) ||
										(strcmp(tmp[3], tmp2[2]) == 0 && strcmp(tmp1[3], tmp2[1]) == 0))) {
									strcpy(circuit[i], tmp2[0]);
									strcat(circuit[i], " ");
		...
		for (int i = 0; i < steps; i++)
		{
			if (strlen(circuit[i]) > 0) printf("%s\n", circuit[i]);
		}
	}
	else
		printf("INVALID\n");
